<template>
  <footer class="footer-section">
    <div class="footer-content">
      <div class="footer-main">
        <!-- Left Column: Original Content -->
        <div class="footer-left">
          <h4 class="resources-title">📚 {{ t('footer.resources')}}</h4>
          <ul class="resources-list">
            <li>
              <a href="https://www.atlanticcouncil.org/cbdctracker/" target="_blank" rel="noopener">
                Atlantic Council CBDC Tracker
              </a>
            </li>
            <li>
              <a href="https://cbdctracker.hrf.org/" target="_blank" rel="noopener">
                Human Rights Foundation CBDC Tracker
              </a>
            </li>
            <li>
              <a href="https://www.bis.org/topics/fintech/cbdc.htm" target="_blank" rel="noopener">
                BIS Central Bank Digital Currencies
              </a>
            </li>
            <li>
              <a href="https://www.imf.org/en/Topics/fintech/central-bank-digital-currency" target="_blank" rel="noopener">
                IMF on CBDCs
              </a>
            </li>
            <li>
              <a href="https://www.ecb.europa.eu/paym/digital_euro/html/index.en.html" target="_blank" rel="noopener">
                European Central Bank - Digital Euro
              </a>
            </li>
            <li>
              <a href="https://www.federalreserve.gov/central-bank-digital-currency.htm" target="_blank" rel="noopener">
                Federal Reserve - CBDC Research
              </a>
            </li>
            <li>
              <a href="https://www.cato.org/topics/monetary-policy/central-bank-digital-currency" target="_blank" rel="noopener">
                Cato Institute - CBDC Analysis
              </a>
            </li>
            <li>
              <a href="https://reason.com/tag/cbdc/" target="_blank" rel="noopener">
                Reason Foundation - CBDC Articles
              </a>
            </li>
            <li>
              <a href="https://bitcoin.org/" target="_blank" rel="noopener">
                Bitcoin.org - Learn About Bitcoin
              </a>
            </li>
            <li>
              <a href="https://www.lopp.net/bitcoin-information.html" target="_blank" rel="noopener">
                Bitcoin Resources by Jameson Lopp
              </a>
            </li>
            <li>
              <a href="https://github.com/bitcoin/bitcoin" target="_blank" rel="noopener">
                Bitcoin Core (GitHub)
              </a>
            </li>
            <li>
              <a href="https://nakamotoinstitute.org/" target="_blank" rel="noopener">
                Satoshi Nakamoto Institute
              </a>
            </li>
          </ul>
          
          
        </div>

        <!-- Middle Column: Resources -->
        <div class="footer-middle">
          <p class="footer-share">{{ t('cta.footer') }}</p>
          
          <div class="footer-links">
            <a href="https://github.com/webiumsk/cbdc-bitcoin" target="_blank" rel="noopener" class="footer-link">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
              </svg>
              GitHub
            </a>
            
            <span class="footer-divider">•</span>
            
            <span class="footer-made">
              {{ t('footer.madeWith') }} <span class="bitcoin-icon">₿</span>
            </span>
          </div>

          <!-- Social Links -->
          <div class="footer-social">
            <a href="https://njump.me/npub18gh7tx4s28dt9t2hhvd6lrwhsmwg94gnxppkezzq0ucqgscav5ysmqwkad" 
               target="_blank" 
               rel="noopener noreferrer"
               class="social-link"
               :title="t('footer.social.nostr')">
              <svg viewBox="0 0 2000 2000" fill="currentColor">
                <path d="M1000 0C447.755 0 0 447.755 0 1000C0 1552.24 447.755 2000 1000 2000C1552.24 2000 2000 1552.24 2000 1000C2000 447.755 1552.24 0 1000 0ZM1574.29 770.408C1532.04 840.612 1445.92 888.367 1437.96 892.449C1421.84 901.02 1415.92 913.469 1416.94 932.041C1417.96 950.612 1413.47 1042.65 1357.35 1083.27C1328.57 1104.08 1212.45 1131.02 1159.18 1140C1131.84 1144.69 1125.51 1153.06 1110.41 1162.04C1090 1174.69 1017.55 1275.1 1005.31 1297.35C1006.94 1297.14 1233.67 1226.73 1243.88 1225.1C1270.82 1221.02 1291.22 1228.78 1304.49 1248.37C1317.14 1267.14 1329.59 1286.12 1342.04 1305.1C1346.53 1311.84 1350.82 318.78 1355.1 1325.71L1359.59 1332.86C1361.22 1335.51 1363.47 1338.57 1364.49 1342.65C1365.31 1345.51 1367.14 1355.1 1361.63 1360.82C1356.53 1366.12 1347.76 1366.12 1343.06 1364.9C1338.57 1363.67 1331.84 1361.02 1326.33 1356.53L1318.78 1350.2C1312.86 1345.1 1301.63 1342.65 1292.86 1343.06C1282.65 1343.47 1269.18 1340.2 1262.65 1336.12C1250 1327.96 1246.73 1307.55 1246.73 1307.35C1245.1 1300.61 1240.2 1298.78 1235.71 1298.78C1227.55 1298.57 1221.22 1301.02 1214.69 1302.86C1184.9 1311.84 1155.31 1321.02 1125.51 1330.41L1082.86 1343.67C1061.84 1350.2 1040.82 1356.73 1020 1363.67C1014.29 1365.51 1008.78 1368.37 1002.86 1371.43C1000.41 1372.65 997.959 1373.88 995.51 1375.1C993.061 1376.33 990.408 1377.76 987.959 1379.18C982.449 1382.24 976.531 1385.51 970 1387.35C948.776 1393.67 930.204 1384.49 921.429 1363.27C915.714 1349.59 920.816 1322.45 923.878 1314.49C943.061 1264.69 1005.51 1170.82 1005.51 1170.2C1004.49 1169.8 936.327 1197.14 925.102 1205.71C894.286 1229.18 822.041 1284.69 821.02 1290C816.327 1314.69 801.02 1333.27 777.551 1342.45C760.612 1348.98 750.408 1362.45 740.408 1375.51C740.408 1375.51 559.388 1624.9 547.143 1638.37C543.469 1642.45 509.592 1677.96 500.612 1694.49C498.571 1698.57 490.816 1714.9 488.776 1718.98C487.143 1722.24 483.469 1731.02 469.592 1730.41C455.714 1729.8 455.306 1710.41 454.694 1702.04C453.265 1681.43 457.755 1661.22 468.571 1640C469.592 1637.96 481.837 1619.39 477.347 1604.49C473.265 1591.43 469.388 1571.22 475.51 1561.02C485.102 1544.9 502.041 1544.29 524.694 1540.61C540.408 1537.96 551.837 1530.2 562.857 1514.08C586.122 1480.2 673.674 1355.92 693.469 1327.14C698.776 1319.59 702.857 1309.8 704.694 1300.2C710.408 1271.22 727.959 1251.02 758.571 1238.57C765.102 1235.92 886.939 1134.08 886.939 1121.02C886.939 1111.63 873.674 1106.53 863.469 1103.67C862.041 1103.27 785.51 1083.47 750.612 1067.55C731.633 1058.98 717.347 1051.43 700.408 1039.18C677.347 1022.24 631.225 1030.41 624.49 1031.43C591.837 1036.12 567.551 1050.82 540 1069.39C535.102 1072.86 510 1081.84 497.347 1074.69C472.653 1060.61 418.367 1035.31 398.367 1004.9C389.184 990.816 390.408 967.959 396.939 942.653C417.959 883.878 448.98 857.551 506.327 841.633C547.143 827.551 629.388 824.49 668.98 821.429C671.633 820.612 759.388 818.571 821.02 786.939C862.041 765.918 983.878 684.898 1159.18 690.408C1230 692.653 1363.88 761.837 1415.51 763.878C1468.16 766.122 1494.49 760.204 1524.9 728.776C1533.47 719.796 1570 641.02 1535.51 597.959C1522.65 582.041 1509.39 567.755 1494.08 554.694C1473.47 537.143 1452.04 520.408 1433.47 500.612C1392.45 457.143 1379.8 397.755 1399.18 351.429C1411.63 318.98 1437.76 304.898 1475.51 312.245C1502.04 317.347 1521.43 341.225 1540.41 353.878C1551.02 361.02 1568.78 365.102 1580.41 367.959C1595.1 371.429 1606.12 382.653 1605.51 390.204C1604.9 397.755 1585.51 404.286 1566.94 402.857C1544.9 401.429 1507.14 401.225 1484.49 408.776C1469.39 414.286 1462.65 431.02 1467.14 447.551C1470.82 461.02 1499.59 485.102 1510.41 494.082C1532.04 511.837 1555.1 527.959 1573.06 550.204C1593.27 575.51 1603.88 604.286 1607.14 636.122C1612.04 684.898 1599.18 728.776 1574.29 770.408Z"/>
              </svg>
            </a>

            <a href="https://twitter.com/Dvadsatjeden" 
               target="_blank" 
               rel="noopener noreferrer"
               class="social-link"
               :title="t('footer.social.twitter')">
              <svg viewBox="0 0 23 24" fill="currentColor">
                <path d="M13.969 10.157l8.738-10.157h-2.071l-7.587 8.819-6.060-8.819h-6.989l9.164 13.336-9.164 10.651h2.071l8.012-9.313 6.4 9.313h6.989l-9.503-13.831zM11.133 13.454l-8.316-11.895h3.181l14.64 20.941h-3.181l-6.324-9.046z"/>
              </svg>
            </a>

            <a href="https://www.facebook.com/Dvadsatjeden.org" 
               target="_blank" 
               rel="noopener noreferrer"
               class="social-link"
               :title="t('footer.social.facebook')">
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M17 3v2h-2c-0.552 0-1.053 0.225-1.414 0.586s-0.586 0.862-0.586 1.414v3c0 0.552 0.448 1 1 1h2.719l-0.5 2h-2.219c-0.552 0-1 0.448-1 1v7h-2v-7c0-0.552-0.448-1-1-1h-2v-2h2c0.552 0 1-0.448 1-1v-3c0-1.105 0.447-2.103 1.172-2.828s1.723-1.172 2.828-1.172zM18 1h-3c-1.657 0-3.158 0.673-4.243 1.757s-1.757 2.586-1.757 4.243v2h-2c-0.552 0-1 0.448-1 1v4c0 0.552 0.448 1 1 1h2v7c0 0.552 0.448 1 1 1h4c0.552 0 1-0.448 1-1v-7h2c0.466 0 0.858-0.319 0.97-0.757l1-4c0.134-0.536-0.192-1.079-0.728-1.213-0.083-0.021-0.167-0.031-0.242-0.030h-3v-2h3c0.552 0 1-0.448 1-1v-4c0-0.552-0.448-1-1-1z"/>
              </svg>
            </a>

            <a href="https://www.youtube.com/@Dvadsatjeden" 
               target="_blank" 
               rel="noopener noreferrer"
               class="social-link"
               :title="t('footer.social.youtube')">
              <svg viewBox="0 0 28 28" fill="currentColor">
                <path d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z"/>
              </svg>
            </a>

            <a href="https://www.instagram.com/dvadsatjeden.slovensko" 
               target="_blank" 
               rel="noopener noreferrer"
               class="social-link"
               :title="t('footer.social.instagram')">
              <svg viewBox="0 0 24 24" fill="currentColor">
                <path d="M7 1c-1.657 0-3.158 0.673-4.243 1.757s-1.757 2.586-1.757 4.243v10c0 1.657 0.673 3.158 1.757 4.243s2.586 1.757 4.243 1.757h10c1.657 0 3.158-0.673 4.243-1.757s1.757-2.586 1.757-4.243v-10c0-1.657-0.673-3.158-1.757-4.243s-2.586-1.757-4.243-1.757zM7 3h10c1.105 0 2.103 0.447 2.828 1.172s1.172 1.723 1.172 2.828v10c0 1.105-0.447 2.103-1.172 2.828s-1.723 1.172-2.828 1.172h-10c-1.105 0-2.103-0.447-2.828-1.172s-1.172-1.723-1.172-2.828v-10c0-1.105 0.447-2.103 1.172-2.828s1.723-1.172 2.828-1.172zM16.989 11.223c-0.15-0.972-0.571-1.857-1.194-2.567-0.754-0.861-1.804-1.465-3.009-1.644-0.464-0.074-0.97-0.077-1.477-0.002-1.366 0.202-2.521 0.941-3.282 1.967s-1.133 2.347-0.93 3.712 0.941 2.521 1.967 3.282 2.347 1.133 3.712 0.93 2.521-0.941 3.282-1.967 1.133-2.347 0.93-3.712zM15.011 11.517c0.122 0.82-0.1 1.609-0.558 2.227s-1.15 1.059-1.969 1.18-1.609-0.1-2.227-0.558-1.059-1.15-1.18-1.969 0.1-1.609 0.558-2.227 1.15-1.059 1.969-1.18c0.313-0.046 0.615-0.042 0.87-0.002 0.74 0.11 1.366 0.47 1.818 0.986 0.375 0.428 0.63 0.963 0.72 1.543zM17.5 7.5c0.552 0 1-0.448 1-1s-0.448-1-1-1-1 0.448-1 1 0.448 1 1 1z"/>
              </svg>
            </a>
            </div>

          <p class="footer-license">Open Source • MIT License • 2025</p>
        </div>

        <!-- Right Column: Lightning Donation -->
        <div class="footer-right">
          <div class="donation-box">
            <img 
              src="/btc-donation-qr.webp" 
              alt="Lightning QR Code" 
              class="qr-image"
            >
            <h4>{{ t('footer.donation.title') }}</h4>
            
            <div class="lightning-address">
              <input 
                ref="addressInput"
                type="text" 
                :value="lightningAddress" 
                readonly 
                class="address-input"
                @click="selectAddress"
              >
              <button @click="copyAddress" class="copy-btn" :title="t('footer.donation.copy')">
                {{ copied ? '✓' : '📋' }}
              </button>
            </div>
            
            <a 
              :href="`lightning:${lightningAddress}`" 
              class="donation-btn"
              target="_blank"
              rel="noopener"
            >
              {{ t('footer.donation.button') }}
            </a>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
import { ref } from 'vue'
import { useI18n } from 'vue-i18n'

const { t } = useI18n()

const lightningAddress = ref('sirius@dvadsatjeden.org')
const copied = ref(false)
const addressInput = ref(null)

const selectAddress = () => {
  if (addressInput.value) {
    addressInput.value.select()
  }
}

const copyAddress = async () => {
  try {
    await navigator.clipboard.writeText(lightningAddress.value)
    copied.value = true
    setTimeout(() => {
      copied.value = false
    }, 2000)
  } catch (err) {
    // Fallback
    selectAddress()
    document.execCommand('copy')
    copied.value = true
    setTimeout(() => {
      copied.value = false
    }, 2000)
  }
}
</script>

<style scoped>
/* Base */
.footer-section {
  background: linear-gradient(135deg, #16213e, #0f3460);
  padding: 50px 20px 30px;
  border-top: 3px solid transparent;
  border-image: linear-gradient(90deg, #e74c3c, #f39c12) 1;
}

.footer-content { max-width: 1400px; margin: 0 auto; }
.footer-main { 
  display: grid; 
  grid-template-columns: 1fr 1fr 1fr; 
  gap: 40px; 
  align-items: center; 
}
.footer-left { display: flex; flex-direction: column; gap: 25px; }

/* Text */
.footer-share, .footer-made { color: #ecf0f1; margin: 0; }
.footer-share { font-size: 1.2rem; }
.footer-made { font-size: 1.1rem; display: flex; align-items: center; gap: 8px; }
.footer-license { font-size: 0.95rem; color: #95a5a6; margin: 0; }
.footer-divider { color: #bdc3c7; font-size: 1.2rem; }

/* Links */
.footer-links { display: flex; align-items: center; gap: 15px; flex-wrap: wrap; }
.footer-link {
  display: flex; align-items: center; gap: 8px;
  color: #f39c12; text-decoration: none;
  font-size: 1.1rem; font-weight: 600;
  padding: 8px 16px; border-radius: 8px;
  background: rgba(243, 156, 18, 0.1);
  transition: all 0.3s;
}
.footer-link:hover { background: rgba(243, 156, 18, 0.2); transform: translateY(-2px); }
.footer-link svg { width: 24px; height: 24px; }

/* Social */
.footer-social { display: flex; gap: 15px; flex-wrap: wrap; }
.social-link {
  width: 40px; height: 40px; border-radius: 50%;
  background: rgba(255, 255, 255, 0.1);
  display: flex; align-items: center; justify-content: center;
  color: #ecf0f1; transition: all 0.3s;
}
.social-link:hover { transform: translateY(-3px); background: rgba(243, 156, 18, 0.3); color: #f39c12; }
.social-link svg { width: 20px; height: 20px; }

/* Bitcoin Icon Animation */
.bitcoin-icon { font-size: 1.3rem; color: #f39c12; font-weight: bold; animation: pulse 2s infinite; }
@keyframes pulse { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } }

/* Resources Column */
.footer-middle {
  display: flex;
  flex-direction: column;
  gap: 15px;
  align-items: center;
}

.resources-title {
  font-size: 1.2rem;
  color: #f39c12;
  margin: 0 0 10px 0;
  font-weight: 600;
}

.resources-list {
  list-style: none;
  padding: 0;
  margin: 0;
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.resources-list li a {
  color: #ecf0f1;
  text-decoration: none;
  font-size: 0.9rem;
  transition: all 0.3s;
  display: block;
  padding: 3px 0;
  border-left: 2px solid transparent;
  padding-left: 10px;
}

.resources-list li a:hover {
  color: #f39c12;
  border-left-color: #f39c12;
  padding-left: 15px;
}

/* Donation Box */
.footer-right { display: flex; justify-content: center; }
.donation-box {
  background: rgba(255, 255, 255, 0.05);
  border: 2px solid rgba(243, 156, 18, 0.3);
  border-radius: 10px; padding: 20px;
  text-align: center; display: flex; flex-direction: column; gap: 12px;
  width: 100%; max-width: 280px;
}
.qr-image { width: 150px; height: 150px; margin: 0 auto; border-radius: 8px; }
.donation-box h4 { font-size: 1.1rem; color: #f39c12; margin: 0; }

/* Lightning Address */
.lightning-address { display: flex; gap: 8px; }
.address-input {
  flex: 1; padding: 8px;
  background: rgba(255, 255, 255, 0.1);
  border: 1px solid rgba(243, 156, 18, 0.3);
  border-radius: 6px; color: #fff;
  font: 0.8rem 'Courier New', monospace;
  cursor: pointer;
}
.address-input:focus { outline: none; border-color: #f39c12; }

/* Buttons */
.copy-btn, .donation-btn {
  background: rgba(243, 156, 18, 0.2);
  border: 1px solid #f39c12;
  border-radius: 6px; color: #f39c12;
  cursor: pointer; transition: all 0.3s;
}
.copy-btn { padding: 8px 12px; font-size: 1rem; min-width: 45px; }
.donation-btn { padding: 10px 20px; font-size: 0.95rem; font-weight: 600; text-decoration: none; }
.copy-btn:hover, .donation-btn:hover { background: rgba(243, 156, 18, 0.3); }
.copy-btn:hover { transform: translateY(-1px); }
.donation-btn:hover { transform: translateY(-2px); }

/* Responsive */
@media (max-width: 1200px) {
  .footer-main { 
    grid-template-columns: 1fr 1fr; 
    gap: 30px; 
  }
  .footer-middle {
    grid-column: 1 / -1;
    max-width: 600px;
    margin: 0 auto;
  }
  .resources-list {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 8px;
  }
}

@media (max-width: 968px) {
  .footer-main { 
    grid-template-columns: 1fr; 
    gap: 40px; 
  }
  .footer-right { order: -1; }
  .donation-box { max-width: 100%; }
  .footer-middle { 
    max-width: 100%;
    grid-column: auto;
  }
  .resources-list {
    grid-template-columns: 1fr;
  }
}

@media (max-width: 768px) {
  .footer-section { padding: 40px 15px 25px; }
  .footer-left { gap: 20px; }
  .footer-share, .footer-made, .footer-link { font-size: 1rem; }
  .footer-license { font-size: 0.85rem; }
  .footer-links { flex-direction: column; align-items: flex-start; gap: 10px; }
  .footer-divider { display: none; }
  .footer-social { gap: 12px; }
  .social-link { width: 36px; height: 36px; }
  .social-link svg { width: 18px; height: 18px; }
  .donation-box { padding: 20px 15px; }
  .qr-image { width: 120px; height: 120px; }
  .donation-box h4 { font-size: 1rem; }
  .address-input { font-size: 0.75rem; }
  .resources-title { font-size: 1.1rem; }
  .resources-list li a { font-size: 0.85rem; }
}
</style>